...
pub fn auto_import(code: &mut String) {
    let mut imports_to_add = Vec::new();
    let mut needs_cpu_mhz = false;
    
    // Check missing imports
    if (code.contains("@delay_ms") || code.contains("@delay_us")) && !code.contains("import delay") {
        imports_to_add.push("import delay");
        needs_cpu_mhz = !code.contains("@cpu_mhz");
    }
    if code.contains("@math_") && !code.contains("import math") {
        imports_to_add.push("import math");
    }
    if code.contains("@adc_") && !code.contains("import adc") {
        imports_to_add.push("import adc");
    }
    if code.contains("@uart_") && !code.contains("import uart") {
        imports_to_add.push("import uart");
    }
    if code.contains("@spi_") && !code.contains("import spi") {
        imports_to_add.push("import spi");
    }
    
    if !imports_to_add.is_empty() || needs_cpu_mhz {
